    ...
    <section className="relative bg-asphalt-deep overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-6 md:px-10 py-20 md:py-28">
        ...
        <p className="text-base md:text-lg text-white/65 max-w-2xl leading-relaxed mb-12">
          ลีกฟุตบอลชุมชนที่เข้าถึงคนโคราชมากที่สุดในรอบหลายปี · {SEASON.matches} แมตช์เดย์ ·{" "}
          {SEASON.clubs} สโมสรทั่วจังหวัด
          <br />
          <span className="text-white/40">Korat Super League {SITE_YEAR}</span>
        </p>
          ...
          {[
            {
              ...
              l: "Live Viewers (Projected)",
              s: `ผู้ชมไลฟ์สดสะสม · อิงจากลีกสด ${SEASON.baseYear}`,
            },
            {
              n: `${(SEASON.maxLive / 1000).toFixed(1)}K`,
              l: "Peak Single Match",
              s: `Projected · Match Day 10 · อิงจากลีกสด ${SEASON.baseYear}`,
            },
            {
              n: `${SEASON.clubs} / ${SEASON.matches}`,
              l: "Clubs / Matchdays",
              s: `ครอบคลุมทั่วจังหวัดนครราชสีมา · ฤดูกาล ${SEASON.dataYear}`,
            },
          ].map((item, i) => (
            <div
              key={item.l}
            ...
            >
              ...
              <div className="text-[11px] text-white/40 mt-1 leading-snug">{item.s}</div>
            </div>
          ))}
        </div>
